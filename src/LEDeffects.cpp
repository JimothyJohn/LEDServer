...
void lightning() {
  ...
  uint16_t ledlen = random8(strandLength-ledstart);                        // Determine length of flash (not to go beyond strandLength-1)
...
void confetti() 
{
  ...
  fadeToBlackBy(leds, strandLength, 10);
  int pos = random16(strandLength);
...
void NeonRacers(uint8_t speed, uint8_t racerNumber) {
  ...
  tealLoc += speed;
  // Shift effect
  if (pinkLoc>=strandLength-speed*2-pinkWidth) {
    pinkLoc = strandLength % pinkLoc;
  }
  if (tealLoc>=strandLength-speed-tealWidth) {
    tealLoc = strandLength % tealLoc;
  }
...
void RGBBouncers(uint8_t speed, uint8_t racerNumber) {
  ...
  if (redLoc<=speed) {
    redDir = 1;
  } else if (redLoc>=strandLength-speed-redWidth) {
    redDir = -1;
  }
  if (greenLoc<=speed*2) {
    greenDir = 1;
  } else if (greenLoc>=strandLength-speed*2-greenWidth) {
    greenDir = -1;
  }
  if (blueLoc<=speed*3) {
    blueDir = 1;
  } else if (blueLoc>=strandLength-speed*3-blueWidth) {
    blueDir = -1;
  }
