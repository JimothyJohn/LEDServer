...
Primary:
- Sync to bpm
- Get rid of hardcoded NUMBER_OF_LEDS # of leds
Secondary:
...
Segment::Segment(uint16_t st, uint16_t w, uint16_t l) {
	width = w;
	length = l;
	// Output for user
...
void Segment::Shift(float sTime, uint16_t distance) {
	...
	if(ClockCheck(&_mClock, _mstpTime)) {
		...
		if(abs(_idx)<distance) {
			_idx += dir;
			loc0 = limitcap(dir, loc0, 0, (length-width));
			loc1 = loc0 + width;
		}
	}
}
...
void Segment::Scan(float sTime, int dir) {
	...
	if(ClockCheck(&_mClock, sTime*1000)) {
		loc0 = limitreset(dir, loc0, 0, length);
		loc1 = loc0+width;
	}
}
...
void Segment::Phase(float sTime, int r_offset, int g_offset, int b_offset) {
	...
	if(ClockCheck(&_cClock, sTime*1000)) {
		...
		cur_red = qadd8(r_offset, cur_red);
		cur_green = qadd8(g_offset, cur_green);
		cur_blue = qadd8(b_offset, cur_blue);
	}
}
...
void Segment::Brighten(float sTime, uint8_t intensity, bool repeat) {
	...
	if(ClockCheck(&_cClock, _cstpTime)) {
		...
		if(cur_val<(tgt_val+intensity-dir)) {
			cur_val = limitcap(dir, cur_val, 0, (tgt_val+intensity));
		} else if(repeat) {
