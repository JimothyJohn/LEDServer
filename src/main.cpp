...
// Changes a section of pixels to a solid color
...
void sawtooth(uint8_t masterPalette, uint8_t BeatsPerMinute) {
  ...
  int cur_led = ((millis() % ms_per_beat) / ms_per_led)%(strandLength); // Using millis to count up the strand, with %NUM_LEDS at the end as a safety factor.
